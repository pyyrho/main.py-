...
def _role_manageable(member: discord.Member, role: discord.Role) -> bool:
    guild = member.guild
    me = guild.me
...
BTN_PAD = "\u2000" * 7  # legado: mantido para compatibilidade visual em embeds antigas
...
class ColorButton(discord.ui.Button):
    def __init__(self, key: str, label: str, emoji_str: str, role_key: str, row: int | None = None):
        # Botões de Nick Color ficam somente com o emoji personalizado.
        # O Discord alinha até 5 botões por linha por padrão; forçamos 3 por linha
        # para manter o painel visualmente parecido com uma grade 3x3.
        super().__init__(
            label=None,
...
class ColorNormalView(discord.ui.View):
    def __init__(self):
        ...
        for index, (key, label, emoji_str, _) in enumerate(CORES_NORMAIS):
            # 9 cores normais: 3 linhas com 3 botões.
            self.add_item(ColorButton(key, label, emoji_str, key, row=index // 3))


class ColorDegradeView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)
        for index, (key, label, emoji_str, _) in enumerate(CORES_DEGRADE):
            # 8 degradês: 3 + 3 + 2, sem texto no botão.
            self.add_item(ColorButton(key, label, emoji_str, key, row=index // 3))
