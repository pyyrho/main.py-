...
class BoasVindasModal(discord.ui.Modal, title="Editar Embed de Boas-vindas"):
    ...
    async def on_submit(self, inter: discord.Interaction):
        ...
        if self.banner_f.value.strip():
            banner = self.banner_f.value.strip()
            if not _looks_like_http_url(banner):
                return await inter.followup.send(
                    embed=error_embed("URL inválida", "Use uma URL começando com `https://` ou `http://`."), ephemeral=True
                )
            fields["welcome_banner"] = banner
...
class Config(commands.Cog):
    ...
    async def cfg_automod(self, inter: discord.Interaction, canal_log: discord.TextChannel = None):
        ...
        if not TOKEN:
            return await inter.followup.send(
                embed=error_embed("BOT_TOKEN ausente", "Configure `BOT_TOKEN` nas variáveis do Railway antes de criar regras."), ephemeral=True
            )
            ...
            for i, kw in enumerate(KEYWORDS_BLOCKS):
                ...
                async with session.post(url, json=payload, headers=headers) as resp:
                    if resp.status in (200, 201):
                        ...
                        log.warning("AutoMod rule failed: guild=%s status=%s", inter.guild.id, resp.status)
