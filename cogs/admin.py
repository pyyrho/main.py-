...
OWNER_ID = int(os.environ.get("OWNER_ID", "695037018127990814"))
...
def _chunk_text(lines: list[str], size: int = 20) -> list[list[str]]:
    return [lines[i:i + size] for i in range(0, len(lines), size)] or [[]]


def _can_send_public(ch: discord.TextChannel, me: discord.Member) -> bool:
...
async def admin_servidores(inter: discord.Interaction):
    ...
    chunks = _chunk_text(linhas, 20)
    embeds = []
    for idx, chunk in enumerate(chunks):
        embed = discord.Embed(
            title=f"📋 Servidores ({len(guilds)}) — Página {idx+1}/{len(chunks)}",
            description="\n".join(chunk) if chunk else "Nenhum servidor conectado.",
            color=0x5865F2,
            timestamp=_now(),
        )
...
async def admin_broadcast(inter: discord.Interaction, mensagem: str):
    ...
    for guild in inter.client.guilds:
        ...
        for ch in guild.text_channels:
            if guild.me and _can_send_public(ch, guild.me):
                canal = ch
                break
...
async def admin_recarregar(inter: discord.Interaction, cog: str):
    ...
    try:
        ...
        log_msg = f"Erro ao recarregar {cog}: {type(e).__name__}: {e}"
        print(log_msg, file=sys.stderr)
        await inter.response.send_message(f"❌ Erro ao recarregar `{cog}`: `{type(e).__name__}`", ephemeral=True)
