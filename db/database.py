...
_CONFIG_TTL = int(os.environ.get("GUILD_CONFIG_CACHE_TTL", "300"))  # padrão: 5 minutos

GUILD_CONFIG_COLUMNS = {
    "log_channel", "ticket_category", "ticket_log", "ticket_banner", "staff_roles",
    "welcome_canal", "welcome_msg", "welcome_banner", "welcome_dm", "welcome_cor",
    "welcome_titulo", "welcome_rodape",
    "xp_canal", "xp_max_level", "xp_ativo", "xp_embed_cor", "xp_embed_banner",
    "xp_embed_titulo", "xp_embed_rodape", "xp_cargo_nivel",
    "counter_channel", "aniv_channel", "logs_channel",
}
...
async def init_pool() -> asyncpg.Pool:
    ...
    A função é idempotente: se o pool já existe, apenas retorna o pool atual.
    Isso evita conexões duplicadas quando o Railway reinicia workers ou quando
    extensões são recarregadas em desenvolvimento.
    """
    global _pool
...
async def close_pool() -> None:
    """Fecha o pool do PostgreSQL com segurança.

    Útil para testes, shutdown limpo e ambientes onde o processo recebe SIGTERM.
    """
    global _pool
    if _pool is not None:
        await _pool.close()
        _pool = None
        _config_cache.clear()
        log.info("[DB] Pool fechado com sucesso.")


def clear_config_cache(guild_id: int | None = None) -> None:
    """Limpa o cache de configuração de um servidor ou de todos os servidores."""
    if guild_id is None:
        _config_cache.clear()
    else:
...
async def _create_tables():
    ...
    async with get_pool().acquire() as conn:
        await conn.execute("""
            ...
            -- Colunas usadas por cogs opcionais. ALTER é seguro e idempotente.
            ...
            CREATE INDEX IF NOT EXISTS xp_data_guild_rank ON xp_data(guild_id, level DESC, xp DESC);
            CREATE INDEX IF NOT EXISTS tickets_guild_created ON tickets(guild_id, created_at);
        """)
...
async def upsert_guild_config(guild_id: int, **fields):
    ...
    Mantém whitelist de colunas para impedir SQL dinâmico acidental. Mesmo que
    os campos venham de comandos internos, validar aqui evita bugs silenciosos
    quando um cog passa um nome incorreto.
    """
    if not fields:
        return

    unknown = set(fields) - GUILD_CONFIG_COLUMNS
    if unknown:
        raise ValueError(f"Campo(s) inválido(s) em guild_config: {sorted(unknown)}")

    # Serializa xp_cargo_nivel para JSONB se presente
    if "xp_cargo_nivel" in fields:
        fields["xp_cargo_nivel"] = json.dumps(
            {str(k): int(v) for k, v in fields["xp_cargo_nivel"].items()}
        )
